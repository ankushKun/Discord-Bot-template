...
class Moderation(commands.Cog):
    ...
    async def ban(self,ctx,u:discord.User=None,reason=None):
        if ctx.message.author.guild_permissions.ban_members:
            ...
            await ctx.send(f"{ctx.message.author.mention} you dont have the ban members perms")
    ...
    async def kick(self,ctx,u:discord.User=None,reason=None):
        if ctx.message.author.guild_permissions.kick_members:
            ...
            await ctx.send(f"{ctx.message.author.mention} you dont have the kick members perms")
    ...
    async def on_message(self,message):
        ...
        with open('banned_words.txt','r') as f:
            banned = f.read().split('\n')[:-1]
            for wrd in banned:
                if wrd in message.content:
                    await message.delete()
                    await ctx.send(f"{message.author.mention} your message contains a banned word")
